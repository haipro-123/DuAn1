...
import javaapplication24.Utilities.DBContext;
...
public class HopThitRes implements InHopThitRes{
    ...
    public ArrayList<HopThit> read(){
        ...
        String sql = "SELECT HOPTHIT.ID, HOPTHIT.TRONGLUONG, HOPTHIT.SOLUONG, HOPTHIT.NGAYSANXUAT, HOPTHIT.TRONGLUONG*LOAITHIT.GIATHEOKG, LOAITHIT.ID, LOAITHIT.TEN, LOAITHIT.GIATHEOKG, LOAITHIT.HSD, HOPTHIT.TrangThai FROM HOPTHIT JOIN LOAITHIT ON HOPTHIT.IDLOAITHIT = LOAITHIT.ID ORDER BY HOPTHIT.ID";
        ...
        try {
            while (rs.next()) {
                ...
                listHT.add(new HopThit(Id, rs.getString(6), rs.getString(7), TrongLuong, SoLuong, rs.getDate(4), rs.getInt(10), TongGia));
            }
            return listHT;
    ...
    public HopThit getHTByMa(String Ma){
        HopThit ht = null;
        String sql = "SELECT HOPTHIT.ID, HOPTHIT.TRONGLUONG, HOPTHIT.SOLUONG, HOPTHIT.NGAYSANXUAT, HOPTHIT.TRONGLUONG*LOAITHIT.GIATHEOKG, LOAITHIT.ID, LOAITHIT.TEN, LOAITHIT.GIATHEOKG, LOAITHIT.HSD, HOPTHIT.TrangThai FROM HOPTHIT JOIN LOAITHIT ON HOPTHIT.IDLOAITHIT = LOAITHIT.ID WHERE HOPTHIT.ID = ? ORDER BY HOPTHIT.ID";
        ...
        try {
            while (rs.next()) {
                ...
                int SoLuong = Integer.parseInt(rs.getString(3));
                int TongGia = Integer.parseInt(rs.getString(5));
                ht = new HopThit(Id, rs.getString(6), rs.getString(7), TrongLuong, SoLuong, rs.getDate(4), rs.getInt(10), TongGia);
            }
            return ht;
    ...
    public int add(HopThit ht) {
        String sql = "INSERT INTO HOPTHIT (IDLOAITHIT, TRONGLUONG, SOLUONG, NGAYSANXUAT, TrangThai) VALUES (?,?,?,?,1)";
